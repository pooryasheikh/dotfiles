...
# every brew install/reinstall/tap/untap/uninstall. On a fresh machine that is
# catastrophic: a single `brew untap` run before bootstrap rewrote this file
# from 115 formulae down to the ~9 that happened to be present, after which
# brew-file "succeeded" having installed nothing at all.
#
# So install from the committed copy, not the working copy. Non-destructive:
# the working copy is left alone and only reported on.
    ...
    if ! cmp -s "$COMMITTED_BREWFILE" "$BREWFILE"; then
        {
            ...
            echo "  Installing from the committed version. If the working copy was"
            echo "  regenerated by brew-wrap and you want it back:"
            echo "      yadm checkout -- .config/brewfile/Brewfile"
            echo
        } >&2
    fi
    HOMEBREW_BREWFILE="$COMMITTED_BREWFILE" brew-file install
